...
def discount(discount_rate, start_date, end_date, price):
        if start_date <= datetime.now() and end_date >= datetime.now():
            discount_price = price - discount_rate * price
...
class ProductService:
    ...
    def get_product_detail(self, conn, params):
        ...
        # 상품의 discount_rate, discount_price 입력
        product_detail['discount_rate'], product_detail['discount_price'] = discount(product_detail['discount_rate'], product_detail['discount_start_date'], product_detail['discount_end_date'], product_detail['price'])

        # 셀러의 다른상품 discount_rate, discount_price 입력
        for product in other_product:
            product['discount_rate'], product['discount_price'] = discount(product['discount_rate'], product['discount_start_date'], product['discount_end_date'], product['price'])
        ...
        result = {
            ...
            # 상품 할인가격
            "dicount_price" : product_detail['discount_price'],
            # 상품 상세정보
    ...
    def get_product_option(self, conn, params):
        ...
        # 데이터베이스에 상품 색상이 존재하는지 확인
        exists = self.product_dao.check_product_color_exists(conn, params)
        if not exists['count']:
            raise DataNotExists('상품 색상이 존재하지 않습니다.', 'product color does not exist')
        
        return self.product_dao.get_product_option(conn, params)
